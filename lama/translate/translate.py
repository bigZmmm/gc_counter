...
def detect_combinable(task, actions):
	...
	for part in oneof.parts:
		if isinstance(part, pddl.Atom):
			current_tag = atom_tag.get(part, -1)
			if (current_tag != -1) and (current_tag != tag):
				return False
			else:
				atom_tag.update({part:tag})
	tag = tag + 1

    for action in actions:
	for atom in action.precondition:
		group_tag = atom_tag.get(atom, -1)
		if (group_tag != -1):
			for condition, fact in action.add_effects:
				current_tag = atom_tag.get(fact, -1)
				if (current_tag == -1):
					atom_tag.update({fact:group_tag})
				else:
					if (current_tag != group_tag):
						return False
			for condition, fact in action.del_effects:
				current_tag = atom_tag.get(fact, -1)
				if (current_tag == -1):
					atom_tag.update({fact:group_tag})
				else:
...
def write_oneof_file(translation_key, task, actions):
	...
	#  print >> oneof_file, "END_ONEOF"
		...
		print oneof_combinable
	    maxlen = 0
	    for oneof in oneofs:
	  	if oneof.get_len() > maxlen:
			maxlen = oneof.get_len()
	    print("maxlen: "+str(maxlen))
	    #oneof_combinable = True 不适用oneof_combinable技术
	    axiom_goal = False
	    oneofsize = len(oneofs)
	    print >> oneof_file, "ONEOF\n",
	    print >> oneof_file, "%d\n" % oneofsize,
